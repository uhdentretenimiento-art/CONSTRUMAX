  ...
  return (
    <>
      ...
      <section className="relative overflow-hidden pb-10 pt-20 md:pb-20 md:pt-28">
        ...
        <div className={`absolute inset-0 opacity-25 ${styles.heroTexture}`} />

        <div className="relative z-10 mx-auto w-full max-w-6xl px-4 text-center">
          <AnimateOnScroll animationType="slide-up">
            <p className={`mb-4 inline-flex items-center rounded-full border px-3 py-1 font-display text-[10px] font-semibold uppercase tracking-[0.2em] backdrop-blur-md md:mb-5 md:px-4 md:py-1.5 md:text-sm md:tracking-[0.24em] ${styles.heroBadge}`}>
              Nuestro Blog
            </p>
          ...
          <AnimateOnScroll animationType="slide-up" delay={100}>
            <div className="mx-auto max-w-5xl rounded-2xl border border-white/20 bg-white/10 p-4 shadow-2xl shadow-construmax-blue/10 backdrop-blur-xl md:rounded-3xl md:p-6">
              <div className="mb-4 flex flex-wrap items-center justify-between gap-3 border-b border-white/15 pb-4 text-left">
                ...
                <p className={`rounded-full border px-3 py-1 text-xs font-semibold ${styles.resultsBadge}`}>
              ...
              <div className="flex flex-col gap-4 md:flex-row">
                <div className="relative flex-1">
                  <Search className={`absolute left-4 top-1/2 h-5 w-5 -translate-y-1/2 ${styles.searchIcon}`} />
                  <Input
                    placeholder="Buscar artículos..."
                    value={searchTerm}
                    onChange={(e) => setSearchTerm(e.target.value)}
                    className={`h-11 rounded-xl border border-transparent bg-white/95 pl-11 text-sm focus-visible:border-construmax-blue focus-visible:ring-2 focus-visible:ring-construmax-blue/20 md:h-12 md:text-base ${styles.controlText}`}
                  />
                </div>
                ...
                <div className="w-full md:w-64">
                  ...
                  >
                    <SelectTrigger className={`h-11 rounded-xl border border-transparent bg-white/95 text-sm md:h-12 md:text-base ${styles.controlText}`}>
                    ...
                    <SelectContent className={`bg-white ${styles.selectContent}`}>
                      <SelectItem
                        value="all"
                        className={`hover:bg-slate-100 focus:bg-slate-100 ${styles.selectItem}`}
                      >
      ...
      <section className="relative pb-14 pt-8 md:pb-20 md:pt-14">
        <div className={`absolute inset-0 ${styles.gridGlow}`} />
      ...
      <section className="relative overflow-hidden pb-14 pt-2 md:pt-4">
        <div className="mx-auto w-full max-w-6xl px-4">
          <AnimateOnScroll animationType="slide-up">
            <div className={`relative overflow-hidden rounded-2xl border border-white/15 px-4 py-10 text-center shadow-2xl shadow-black/35 md:rounded-3xl md:px-10 md:py-12 ${styles.ctaPanel}`}>
              ...
              <div className="relative z-10">
                ...
                <Button
                  asChild
                  size="lg"
                  className={`w-full rounded-full bg-blue-700 px-8 py-6 text-base font-bold text-white transition hover:bg-slate-700 sm:w-auto md:px-10 md:py-7 md:text-lg ${styles.ctaButton}`}
                >
